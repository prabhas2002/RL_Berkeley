...
def run_training_loop(params):
    ...
    env = gym.make(params['env_name'], new_step_api=True, render_mode=None)
    env.reset(seed=seed)
    ...
    for itr in range(params['n_iter']):
        ...
        # Generate random indices for the samples
          indices = np.random.permutation(len(replay_buffer.obs))[:num_samples]

        # Get the observations and actions for the sampled indices
