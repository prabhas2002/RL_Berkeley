...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            #next_action = ...
            next_obs = torch.Tensor(next_obs).to(ptu.device)
        ...
        # Predict Q-values
        #q_values = ...
        obs= torch.Tensor(obs).to(ptu.device)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            #q_values = ...
            q_values = self.critic(obs[None].expand(self.num_critic_networks, -1, -1), action)
            ...
            q_values = torch.mean(q_values, axis=0)  #to reduce the varience.
            advantage = q_values
    ...
    def update(
        ...
        bools=False
        if self.soft_target_update_rate and step % self.soft_target_update_rate == 0:
            self.soft_update_target_critic(self.soft_target_update_rate)
            bools=True
            
        #hard update
        if bools==False and self.target_update_period and  step % self.target_update_period==0:
            self.update_target_critic()
